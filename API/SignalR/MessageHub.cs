...
public class MessageHub(IUnitOfWork uow, IHubContext<PresenceHub> presenceHub) : Hub
{
    public override async Task OnConnectedAsync()
    {
        ...
        var groupName = GetGroupName(GetUserId(), otherUser).ToString();
        await Groups.AddToGroupAsync(Context.ConnectionId, groupName);
        await AddToGroup(groupName);

        var messages = await uow.MessageRepository.GetMessageThread(
            GetUserId(),
            otherUser
        );
    ...
    public async Task SendMessage(CreateMessageDto createMessageDto)
    {
        ...
        var groupName = GetGroupName(sender.Id, recipient.Id).ToString();
    ...
    private object GetGroupName(string? caller, string other)
    {
    ...
    {
        return Context.User?.GetMemberId() ?? throw new Exception("User ID not found in context");
    }
}
